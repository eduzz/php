set -x \
    && apt-get update \
    && apt-get install -y libpq-dev uuid-dev libsodium-dev libzip-dev librabbitmq-dev libssh-dev libfreetype6-dev libjpeg62-turbo-dev libpng-dev libxml2-dev libsybdb5 freetds-dev locales gnupg2 apt-transport-https \
    ...
    && pecl install libsodium mongodb amqp redis uuid sqlsrv-5.10.0 pdo_sqlsrv-5.10.0 \
    ...
    && useradd www-data -u 1000 -g 1000